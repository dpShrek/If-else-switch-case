...
function f1() {
  let ferst = document.querySelector(".i-1").value;
...
function f8() {
  ...
  switch (s8) {
    ...
    case "3":
      ...
      break;
    default:
      res.innerHTML = "Нет таких значений";
  }
}
...
// Дана кнопка b-14, при нажатии на которую срабатывает функция f14. Функция выводит в .out-14 результат операций выбранной в 3-м select к числам введенным
// в первом и втором input. Например выбрано 1 13 +, нужно вывести результат операции 1+13 т.е.  14.

function f14() {
  let i141 = Number(document.querySelector(".i-141").value);
  let i142 = Number(document.querySelector(".i-142").value);
  let i143 = document.querySelector(".s-143").value;
...
function f15() {
  let i151 = Number(document.querySelector(".s-151").value);
  let i152 = Number(document.querySelector(".s-152").value);
  let i153 = document.querySelector(".s-153").value;
